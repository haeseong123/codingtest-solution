...
# <문제 풀이>
# dungeons의 개수가 얼마 되지 않으니 permutaions로 모든 조합의 배열을 구하고
# 해당 배열을 순회하며 각각 몇개의 던전을 돌 수 있는지 cnt한뒤, cnt의 최댓값을 반환하면 된다.

# 1. dungeons를 조합하는 permutations()를 작성한다.
# 2. for i in range(1, len(dungeons)):
#       permutation(0, i)
# 3. 2의 결과로 얻은 배열을 순회하며 get_cnt(e)를 호출한다.
#       결과로 얻은 cnt를 _max와 비교한다.
# 4. _max를 반환한다.
...
def permutations(start, n, r, res):
    if start == r:
        res.append(n[:r])
        return
    ...
    for i in range(start, len(n)):
        ...
        permutations(start + 1, n, r, res)
        n[start], n[i] = n[i], n[start]

def get_cnt(k, dungeons):
    cnt = 0
    for required, consumed in dungeons:
        if k < required:
            break
        cnt += 1
        k -= consumed
...
def solution(k, dungeons):
    ...
    for i in range(1, len(dungeons) + 1):
        permutations(0, dungeons, i, ps)
    
    for p in ps:
        answer = max(answer, get_cnt(k, p))
